...
app.retrieveGif = (iconPhrase) => {
    fetch(`https://api.giphy.com/v1/gifs/search?q=weather+`+iconPhrase+`&api_key=${app.giphyApiKey}`)
        .then(function (response) {
            return response.json();
        })
        .then((gifArray) => {
            ...
            const chosenGifs = [app.randomizer(obtainedGifs)]
            console.log(chosenGifs)

            chosenGifs.forEach((gif) =>{
